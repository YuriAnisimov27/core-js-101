...
function isLeapYear(date) {
  ...
  return !!(data % 400 === 0 || (data % 4 === 0 && data % 100 !== 0));
}
...
function timeSpanToString(startDate, endDate) {
  const MILLISECONDS_IN_DAY = 3600000;
  const MILLISECONDS_IN_MINUTE = 60000;
  const timestampDiff = endDate - startDate;

  const addZero = (data) => (data < 10 ? `0${data}` : data);

  const hours = Math.floor(timestampDiff / MILLISECONDS_IN_DAY);
  const minutes = Math.floor((timestampDiff % MILLISECONDS_IN_DAY) / MILLISECONDS_IN_MINUTE);
  let seconds = timestampDiff % MILLISECONDS_IN_MINUTE;
  seconds = (seconds / 1000).toFixed(3);
